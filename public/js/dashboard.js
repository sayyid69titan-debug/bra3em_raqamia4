(function(){
  ...
  function updateHero(summary){
    ...
    textEls.forEach(el=>el.textContent = `أُنجز ${summary.doneItems} عنصرًا بين أنشطة وألعاب مع حفظ فعلي داخل المشروع.`);
  }
  ...
  async function boot(){
    ...
    window.BaraemProgress.subscribe((payload)=>{
      ...
      if (window.BaraemHome && window.BaraemHome.updateMission) window.BaraemHome.updateMission(summary);
